...
function hangmanApp(div_id, questions, answers, count, layout) {
    ...
    //Imposing default value assignment for count.
    ...
    //Imposing default vallue for layout
    if(count!=undefined && layout===undefined){
		if(count===layoutEnum.REGULAR || count===layoutEnum.TALL || count===layoutEnum.WIDE){
			setData.layout = count;
		}else{
			   setData.layout = layoutEnum.REGULAR;
			}
	}else{
		if(layout===layoutEnum.REGULAR || layout===layoutEnum.TALL || layout===layoutEnum.WIDE){
